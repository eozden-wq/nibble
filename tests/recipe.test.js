  ...
  describe("POST /api/recipe/img/add", () => {
    beforeEach(() => {
      mock_fs({
        ...
        "server/data/recipe_imgs": {},
        "tests/uploads": {
          imgs: {
            "test.jpg": "fake-image",
          },
          "file.txt": "This is supposed to get rejected",
        },
      });
    });
    ...
    test("api.recipe.img.add should give a 200 response for a valid image being uploaded", async () => {
      const response = await request(app)
        .post("/api/recipe/img/add")
        .query({ id: 0 })
        .attach("recipe_img", "tests/uploads/imgs/test.jpg");

      expect(response.statusCode).toBe(200);
      expect(response.body.message).toBe("Success");
    });
    test("api.recipe.img.add should give a 400 response for a non-image file being uploaded", async () => {
      const response = await request(app)
        .post("/api/recipe/img/add")
        .query({ id: 0 })
        .attach("recipe_img", "tests/uploads/file.txt");

      expect(response.statusCode).toBe(400);
      expect(response.body.message).toBe("Malformed request");
    });
    test("api.recipe.img.add should give a 400 response for a valid image with invalid recipe_id", async () => {
      const response = await request(app)
        .post("/api/recipe/img/add")
        .query({ id: -100000 })
        .attach("recipe_img", "tests/uploads/imgs/test.jpg");
    ...
    test("api.recipe.img.add should give a 400 response for an invalid image with invalid recipe_id", async () => {
      const response = await request(app)
        .post("/api/recipe/img/add")
        .query({ id: -1000000 })
        .attach("recipe_img", "tests/uploads/file.txt");
    ...
    test("api.recipe.img.add should give a 400 response if an image isn't uploaded for a valid id", async () => {
      const response = await request(app)
        .post("/api/recipe/img/add")
        .query({ id: 0 });

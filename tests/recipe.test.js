...
describe('Testing Recipe-related API Endpoints', () => {
    ...
    describe('POST /api/recipe/create', () => {
        test('api.recipe.create should return 200 when a valid recipe is given', () => {
            request(app).post('/api/recipe/create').send({"author": "John Doe", "dish_name": "Sticky Toffee Pudding", "instructions": "I don't know how to make Sticky toffee pudding"}).set('Content-Type', 'application/json').expect({"code": 200, "message": "Success"})
        });
    });
});